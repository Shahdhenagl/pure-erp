...
function Storefront({ cart, onBack, onAdd }: { cart: string[]; onBack: () => void; onAdd: (name: string) => void }) {
  type StoreView = 'home' | 'product' | 'offers' | 'cart' | 'checkout' | 'success';
  ...
  const submitOrder = async () => {
    ...
    setSubmitting(true);
    const { error } = await supabase.rpc('submit_public_order', { p_business_name: form.businessName, p_contact_name: form.contactName, p_phone: form.phone, p_address: form.address, p_latitude: location?.lat ?? null, p_longitude: location?.lng ?? null, p_notes: form.notes, p_items: items });
    setSubmitting(false);
...
function HeroSlidesSettings() {
  ...
  const addSlide = async () => {
    ...
    if (file) { const bytes = new Uint8Array(await file.arrayBuffer()); let binary = ""; bytes.forEach((byte) => { binary += String.fromCharCode(byte); }); const uploaded = await uploadImage.mutateAsync({ fileName: file.name, contentType: file.type as "image/png" | "image/jpeg" | "image/jpg" | "image/webp", dataBase64: btoa(binary) }); finalUrl = uploaded.url; }
...
function ProductImagesSettings() {
  ...
  const upload = async () => { if (!selected || !file) return toast.error("اختر المنتج والصورة أولاً"); const bytes = new Uint8Array(await file.arrayBuffer()); let binary = ""; bytes.forEach((byte) => { binary += String.fromCharCode(byte); }); const result = await uploadImage.mutateAsync({ fileName: file.name, contentType: file.type as "image/png" | "image/jpeg" | "image/jpg" | "image/webp", dataBase64: btoa(binary) }); const { error } = await supabase.from("products").update({ image_url: result.url }).eq("id", selected); if (error) return toast.error("تعذر حفظ صورة المنتج"); setItems((rows) => rows.map((row) => row.id === selected ? { ...row, image_url: result.url } : row)); setFile(null); toast.success("تم تحديث صورة المنتج"); };
