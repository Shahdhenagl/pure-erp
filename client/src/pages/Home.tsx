...
function Storefront({ cart, onBack, onAdd }: { cart: string[]; onBack: () => void; onAdd: (name: string) => void }) {
  ...
  const [slides, setSlides] = useState([
    { title: "طعم الفاكهة الأصلي", subtitle: "مع جيلي بيور", cta_label: "تسوق الآن", image_url: "https://files.manuscdn.com/user_upload_by_module/session_file/310519663954061026/qxrfvCMOCddzDFwM.jpg" },
    { title: "اختيارات بيور المميزة", subtitle: "منتجات طبيعية لكل بيت", cta_label: "اكتشف المنتجات", image_url: "https://files.manuscdn.com/user_upload_by_module/session_file/310519663954061026/cCQKbdhEJnsccuEp.jpg" },
    { title: "موسم الطعم الطبيعي", subtitle: "عروض خاصة لفترة محدودة", cta_label: "شاهد العروض", image_url: "https://files.manuscdn.com/user_upload_by_module/session_file/310519663954061026/TNfgEfVNlDPfdVnZ.jpg" },
  ]);
  const [activeSlide, setActiveSlide] = useState(0);
  useEffect(() => {
    let active = true;
    supabase.from("products").select("name, sku, sale_price, product_type").eq("is_active", true).eq("product_type", "finished").order("name").then(({ data, error }) => {
      if (!active || error || !data?.length) return;
      setCatalog(data.map((product, index) => ({ name: product.name, sku: product.sku, price: Number(product.sale_price || 0).toFixed(2), stock: 0, color: ["#c7352f", "#eb9d19", "#5d953b", "#e24f38", "#d3bd39"][index % 5] })));
    });
    return () => { active = false; };
  }, []);
  useEffect(() => {
    let active = true;
    supabase.from("store_hero_slides").select("title, subtitle, cta_label, image_url").eq("is_active", true).order("sort_order").then(({ data }) => {
  ...
  const categories = [{ icon: "◌", label: "منتجات طبيعية" }, { icon: "◒", label: "معلبات" }, { icon: "▱", label: "بقوليات ومكسرات" }, { icon: "◍", label: "بهارات وخلطات" }, { icon: "▤", label: "سناكس" }, { icon: "♧", label: "جيلي" }];
  return <div className="store-shell" dir="rtl">
    <header className="store-topbar"><div className="store-brand-lockup"><img className="store-real-logo" src="https://files.manuscdn.com/user_upload_by_module/session_file/310519663954061026/sHJXxOuFhntaMcJc.png" alt="بيور اختياراتك الطبيعية" /></div><nav className="store-links"><button className="active">الرئيسية</button><button>المنتجات</button><button>العروض</button><button>من نحن</button><button>تواصل معنا</button></nav><div className="store-search"><Search size={17} /><span>إبحث عن منتجك المفضل...</span></div><div className="store-actions"><button className="store-account"><UserCog size={18} /><span>حسابي</span></button><button className="store-cart"><ShoppingBag size={20} /><span>{cart.length}</span></button><button className="store-mobile-menu"><Menu size={20} /></button></div></header>
    <main className="store-content">
      <section className="store-hero-reference"><div className="hero-overlay-copy"><div className="store-logo-watermark">بيور</div><h1>{slides[activeSlide]?.title}</h1><p>{slides[activeSlide]?.subtitle}</p><span>بدون أي إضافات صناعية</span><button onClick={() => document.getElementById("store-products")?.scrollIntoView({ behavior: "smooth" })}>{slides[activeSlide]?.cta_label} <ArrowLeft size={16} /></button></div><img src={slides[activeSlide]?.image_url} alt={slides[activeSlide]?.title} /><div className="hero-controls"><button onClick={() => setActiveSlide((activeSlide - 1 + slides.length) % slides.length)}><ArrowRightLeft size={15} /></button><div>{slides.map((_, index) => <button key={index} className={index === activeSlide ? "active" : ""} onClick={() => setActiveSlide(index)} />)}</div><button onClick={() => setActiveSlide((activeSlide + 1) % slides.length)}><ArrowLeft size={15} /></button></div></section>
      <section className="category-strip">{categories.map((category) => <button key={category.label} onClick={() => document.getElementById("store-products")?.scrollIntoView({ behavior: "smooth" })}><span>{category.icon}</span><b>{category.label}</b></button>)}</section>
      <section id="store-products" className="store-product-section"><div className="store-section-heading"><div><span>اختيارات بيور</span><h2>منتجاتنا <em>المميزة</em></h2></div><button onClick={() => toast.info("كل المنتجات متاحة في الكتالوج")}>عرض الكل <ArrowLeft size={15} /></button></div><div className="product-grid">{catalog.map((product) => <article className="product-card-reference" key={product.sku}><div className="product-photo" style={{ background: "linear-gradient(145deg, " + product.color + "22, #f8faf7)" }}><div className="product-pack"><span>بيور</span><b>{product.name.split(" ").slice(0, 2).join(" ")}</b><i>طبيعي 100%</i></div></div><h3>{product.name}</h3><p>{product.name.includes("كريم") ? "80 جم جاهز" : product.name.includes("جيلي") ? "70 جم" : "250 جم"}</p><strong>{product.price} <small>ج.م</small></strong><button className="reference-add" onClick={() => onAdd(product.name)}>أضف إلى السلة <ShoppingBag size={14} /></button></article>)}</div></section>
      <section className="store-benefits-reference"><div><ShieldCheck size={24} /><b>منتجات طبيعية 100%</b><span>جودة تستحقها</span></div><div><Truck size={24} /><b>شحن سريع</b><span>لجميع أنحاء الجمهورية</span></div><div><CheckCircle2 size={24} /><b>دفع آمن</b><span>بجميع وسائل الدفع</span></div><div><Smartphone size={24} /><b>خدمة عملاء متميزة</b><span>نحن هنا لمساعدتك دائماً</span></div></section>
    </main><footer className="store-footer-reference"><div className="footer-brand"><img className="store-real-logo footer-real-logo" src="https://files.manuscdn.com/user_upload_by_module/session_file/310519663954061026/sHJXxOuFhntaMcJc.png" alt="بيور" /><span>اختياراتك الطبيعية</span><small>© 2025 PURE - جميع الحقوق محفوظة</small></div><div><b>روابط سريعة</b><span>الرئيسية</span><span>المنتجات</span><span>العروض</span><span>من نحن</span></div><div><b>أقسام المنتجات</b><span>جيلي وسناكس</span><span>بهارات وخلطات</span><span>معلبات</span><span>منتجات طبيعية</span></div><div className="footer-news"><b>تابعنا على</b><span>اشترك في نشرتنا البريدية</span><div><input placeholder="أدخل بريدك الإلكتروني" /><button><Send size={15} /></button></div></div></footer>
  </div>;
}
