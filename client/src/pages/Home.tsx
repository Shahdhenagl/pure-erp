...
function Storefront({ cart, onBack, onAdd }: { cart: string[]; onBack: () => void; onAdd: (name: string) => void }) {
  ...
  useEffect(() => {
    let active = true;
    supabase.from('products').select('id, name, sku, sale_price, product_type').eq('is_active', true).eq('product_type', 'finished').order('name').then(({ data }) => {
      if (active && data?.length) setCatalog(data.map((product, index) => ({ id: product.id, name: product.name, sku: product.sku, price: Number(product.sale_price || 0).toFixed(2), stock: 0, color: ['#c7352f', '#eb9d19', '#5d953b', '#e24f38', '#d3bd39'][index % 5] })));
    });
  ...
  const submitOrder = async () => {
    ...
    setSubmitting(true);
    const { error } = await supabase.rpc('submit_public_order', { p_business_name: form.businessName, p_contact_name: form.contactName, p_phone: form.phone, p_address: form.address, p_notes: form.notes, p_items: items });
    setSubmitting(false);
  ...
  const ProductCard = ({ product }: { product: StoreProduct }) => <article className="product-card-reference"><button className="product-photo" onClick={() => goProduct(product)}><div className="product-pack"><span>بيور</span><b>{product.name.split(' ').slice(0, 2).join(' ')}</b><i>طبيعي 100%</i></div></button><button className="product-title-button" onClick={() => goProduct(product)}><h3>{product.name}</h3><p>{product.name.includes('كريم') ? '80 جم جاهز' : product.name.includes('جيلي') ? '70 جم' : '250 جم'}</p></button><strong>{product.price} <small>ج.م</small></strong><button className="reference-add" onClick={() => addAndStay(product)}>أضف إلى السلة <ShoppingBag size={14} /></button></article>;
  ...
  const CheckoutPage = () => <section className="store-inner-page"><div className="store-section-heading"><div><span>خطوة أخيرة</span><h2>بيانات <em>التوصيل</em></h2></div><button onClick={() => nav('cart')}>السلة <ArrowLeft size={15} /></button></div><div className="checkout-layout"><form className="checkout-form" onSubmit={(event) => { event.preventDefault(); submitOrder(); }}><label>اسم المتجر*<input value={form.businessName} onChange={(event) => setForm({ ...form, businessName: event.target.value })} placeholder="مثال: سوبر ماركت الإيمان" required /></label><label>اسم المسؤول<input value={form.contactName} onChange={(event) => setForm({ ...form, contactName: event.target.value })} placeholder="الاسم بالكامل" /></label><label>رقم الهاتف*<input type="tel" value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} placeholder="01xxxxxxxxx" required /></label><label>عنوان التوصيل*<textarea value={form.address} onChange={(event) => setForm({ ...form, address: event.target.value })} placeholder="المحافظة، المنطقة، الشارع والعلامة المميزة" required /></label><label>ملاحظات الطلب<textarea value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} placeholder="أي ملاحظات للمندوب" /></label><button className="primary-button full-width" disabled={submitting}>{submitting ? 'جاري إرسال الطلب...' : 'تأكيد وإرسال الطلب'}</button></form><div className="checkout-summary"><h3>ملخص الطلب</h3>{cartProducts.map((product, index) => <div key={product.sku + index}><span>{product.name}</span><b>{product.price} ج.م</b></div>)}<hr /><strong>الإجمالي {total.toFixed(2)} ج.م</strong><p><ShieldCheck size={15} /> بياناتك محفوظة وتصل مباشرة لفريق PURE</p></div></div></section>;
...
function HeroSlidesSettings() {
  ...
  const addSlide = async () => {
    if (!title.trim() || !imageUrl.trim()) return toast.error("اكتب عنوان العرض ورابط الصورة");
    const row = { title: title.trim(), image_url: imageUrl.trim(), is_active: true };
    const { error } = await supabase.from("store_hero_slides").insert(row);
    if (error) return toast.error("تعذر حفظ الشريحة — تأكد من دخول المدير");
    setSlides((items) => [...items, row]); setTitle(""); setImageUrl(""); toast.success("تمت إضافة شريحة Hero جديدة");
  };
  return <div className="panel hero-slides-panel"><PanelHeader title="صور الهيرو والعروض" meta="إدارة المتجر" icon={Store} /><p className="settings-help">أضف أكثر من صورة للبانر، وسيتم تدويرها تلقائياً في متجر التجار.</p><div className="hero-slide-list">{slides.map((slide, index) => <div className="hero-slide-row" key={slide.image_url + index}><img src={slide.image_url} alt="" /><div><b>{slide.title}</b><span>{slide.is_active ? "نشطة في المتجر" : "متوقفة"}</span></div><button className="more-button" onClick={() => setSlides((items) => items.filter((_, itemIndex) => itemIndex !== index))}><X size={14} /></button></div>)}</div><div className="hero-slide-form"><input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="عنوان العرض" /><input value={imageUrl} onChange={(event) => setImageUrl(event.target.value)} placeholder="رابط صورة Hero عالية الجودة" /><button className="primary-button" onClick={addSlide}><Plus size={15} /> إضافة صورة</button></div></div>;
}
