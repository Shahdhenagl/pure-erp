...
const navItems: { key: PageKey; label: string; icon: typeof LayoutDashboard; badge?: string }[] = [
  ...
  { key: "products", label: "المنتجات والكميات", icon: Package },
...
function Storefront({ cart, onBack, onAdd }: { cart: string[]; onBack: () => void; onAdd: (name: string) => void }) {
  ...
  useEffect(() => {
    let active = true;
    supabase.from('erp_products').select('id, name, sku, sale_price, product_type, image_url, discount_percent').eq('is_active', true).eq('product_type', 'finished').order('name').then(({ data }) => {
  ...
  const submitOrder = async () => {
    ...
    const { error: stockError } = await supabase.rpc('erp_finalize_sale', { p_items: items, p_reference: createdOrder?.order_number || 'بيع متجر' });
    if (stockError) toast.warning('تم تسجيل الطلب، لكن لم يتم خصم المخزون: ' + stockError.message);
...
function ProductImagesSettings() {
  ...
  useEffect(() => { supabase.from("erp_products").select("id, name, image_url").eq("product_type", "finished").order("name").then(({ data }) => { if (data) setItems(data); }); }, []);
  const upload = async () => { if (!selected || !file) return toast.error("اختر المنتج والصورة أولاً"); const compressed = await compressImage(file); const result = await uploadImage.mutateAsync(compressed); const { error } = await supabase.from("erp_products").update({ image_url: result.url }).eq("id", selected); if (error) return toast.error("تعذر حفظ صورة المنتج"); setItems((rows) => rows.map((row) => row.id === selected ? { ...row, image_url: result.url } : row)); setFile(null); toast.success("تم تحديث صورة المنتج"); };
